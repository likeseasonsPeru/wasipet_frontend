...
const TableProducts = () =>{
    ...
    const formatProducts = async (products) => {
       let newDownload = await products.map(product => {
            ...
            newObj['Foto'] = `https://wasipetapp.com/api/public/${product.type}`
            return newObj;
        })
                ...
                contentArray.map((product,index) => {
                    return(
                        <TableRow hover  key={product._id}>
                            ...
                            {
                                    product.type ? 
                                    <img src={`https://wasipetapp.com/api/public/${product.type}`} style={{width:'40px'}}/>
                                    : 'Sin foto'
                                }
                            </TableCell>
                            <TableCell key={Math.random()} align="center">
